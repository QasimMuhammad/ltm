...
function ParentQuestionnaire() {
  ...
  const { data: questionsData, isLoading, error } = useQuery({
    ...
    queryFn: () => questionsApi.getAll('parent'),
    select: (data) => data.data.questions,
  });
  
  const submitMutation = useMutation({
    mutationFn: responsesApi.submitParent,
    onSuccess: () => {
      navigate('/thank-you');
    },
    onError: (error) => {
  ...
  return (
    <div className="questionnaire-container">
      <h1 style={{ marginBottom: '30px', color: 'var(--primary-color)' }}>Parent Questionnaire</h1>
      <p style={{ marginBottom: '30px', color: '#666' }}>
        As parents, your perspective is valuable. Please share your thoughts on the upcoming marriage.
      </p>
